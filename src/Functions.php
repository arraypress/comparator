...
namespace {
	...
	if ( ! function_exists( 'comparator' ) ) {
		 ...
		 */
		function comparator( $value1, $value2, string $operator, ?string $type = null, bool $caseSensitive = true, bool $useEpsilon = false, string $matchType = 'all', ?callable $customFunction = null, ?callable $errorCallback = null ): ?bool {
			try {
				// Instantiate the Comparator with the specified type and case sensitivity
				$comparator = new Comparator( $type, $caseSensitive, $matchType );
